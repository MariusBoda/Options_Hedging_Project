...
def check_ric(ric, maturity, ident, interval=rdp.Intervals.DAILY):
    exp_date = pd.Timestamp(maturity)

    # Adjust date range based on interval (intraday data needs shorter periods)
    if interval in [rdp.Intervals.ONE_MINUTE, rdp.Intervals.FIVE_MINUTES, rdp.Intervals.TEN_MINUTES,
                   rdp.Intervals.THIRTY_MINUTES, rdp.Intervals.ONE_HOUR]:
        days_back = 30  # Shorter period for intraday data
    else:
        days_back = 90  # Longer period for daily data

    # get start and end date for get_historical_price_summaries query
    sdate = (datetime.now() - timedelta(days_back)).strftime('%Y-%m-%d')
    edate = datetime.now().strftime('%Y-%m-%d')

    # check if option is matured. If yes, add expiration syntax
    # and recalculate start and end date of the query (take expiration day minus days_back period)
    if pd.Timestamp(maturity) < datetime.now():
        ric = ric + '^' + ident[str(exp_date.month)]['exp'] + str(exp_date.year)[-2:]
        sdate = (exp_date - timedelta(days_back)).strftime('%Y-%m-%d')
        edate = exp_date.strftime('%Y-%m-%d')

    # request option prices. Please note, there is no settle price for OPRA traded options
    if ric.split('.')[1][0] == 'U':
        prices = rdp.get_historical_price_summaries(ric,  start = sdate, end = edate, interval = interval,
...
def get_ric_opra(asset, maturity, strike, opt_type, interval=rdp.Intervals.DAILY):
    exp_date = pd.Timestamp(maturity)
    ...
    # check ric validity
    ric, prices = check_ric(ric, maturity, ident, interval)
...
def get_ric_hk(asset, maturity, strike, opt_type, interval=rdp.Intervals.DAILY):
    exp_date = pd.Timestamp(maturity)
...
def get_ric_ose(asset, maturity, strike, opt_type, interval=rdp.Intervals.DAILY):
    exp_date = pd.Timestamp(maturity)
...
def get_ric_eurex(asset, maturity, strike, opt_type, interval=rdp.Intervals.DAILY):
    exp_date = pd.Timestamp(maturity)
...
def get_ric_ieu(asset, maturity, strike, opt_type, interval=rdp.Intervals.DAILY):
    exp_date = pd.Timestamp(maturity)
...
def get_optionRic(isin, maturity, strike, opt_type, interval=rdp.Intervals.DAILY):
    ...
    # Special handling for SPY ETF
    if isin == 'US78462F1030':  # SPY ISIN
        ricUnderlying = 'SPY'  # Use SPY directly
    else:
        # convert ISIN to RIC
    ...
    for exch in exchnage_codes:
        if exch in exchanges.keys():
            ric, prices = exchanges[exch](ricUnderlying, maturity, strike, opt_type, interval)
            if prices is not None:
